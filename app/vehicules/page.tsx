...
/* Options dérivées du catalogue (tous les véhicules, pas seulement visibles) */
const ALL_BRANDS = Array.from(
	new Set(allVehiclesRaw.map((v) => v.brand)),
).sort();
const ALL_FUELS = Array.from(new Set(allVehiclesRaw.map((v) => v.fuel)));
			...
			{filtered.length > 0 ? (
				...
				<div className="text-center py-24 bg-white rounded-2xl border border-slate-200">
					...
					<p className="text-[#475569] text-sm mb-6 max-w-xs mx-auto">
						Essayez d&apos;élargir votre recherche en modifiant ou
						en supprimant certains filtres.
					</p>
					<button
