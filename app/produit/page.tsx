        ...
        <div className="container mx-auto px-4 relative">
          <div className="max-w-3xl">
            ...
            <p className="text-slate-300 text-xl leading-relaxed max-w-2xl mb-10">
              Chaque véhicule de notre stock est inspecté selon 160 points de
              contrôle, révisé par nos mécaniciens et garanti. Financement et
              reprise étudiés ensemble.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/vehicules" className="btn-primary text-base py-4 px-8">
                <Car size={18} />
                Voir tout le stock
              </Link>
              <a href="tel:0532002038" className="btn-outline text-base py-4 px-8">
                <Phone size={18} />
                Appeler le garage
              </a>
            </div>
            ...
            <div className="flex items-end justify-between mb-10">
              <div>
                ...
                <h2 className="section-title">
                  Véhicules{" "}
                  <span className="text-brand-500">à la une</span>
                </h2>
              </div>
              <Link
                href="/vehicules"
        ...
        <section className="py-20 bg-white">
          <div className="container mx-auto px-4">
            <div className="flex items-end justify-between mb-10">
              <div>
                <div className="section-divider" />
                <span className="eyebrow">Disponibles maintenant</span>
                <h2 className="section-title">Autres véhicules</h2>
              </div>
            </div>
            ...
            <div className="text-center">
              <Link href="/vehicules" className="btn-primary text-base px-10 py-4">
                <Car size={18} />
                Voir tous les véhicules ({vehicles.length} disponibles)
              </Link>
            </div>
          </div>
        </section>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center max-w-3xl mx-auto">
            {[
              ...
              <div key={label}>
                <div className="font-heading font-black text-4xl text-brand-500 mb-1">{value}</div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              { name: "Thomas R.", note: "Achat d'une Volkswagen Golf. Véhicule impeccable, révision faite, garantie 12 mois. Je recommande vivement !" },
              { name: "Marie-Claire D.", note: "Service au top, transparent sur les prix. La voiture était comme neuve. Merci à toute l'équipe !" },
              { name: "Karim B.", note: "Troisième véhicule acheté ici. Toujours aussi sérieux. Financement rapide et sans surprise." },
            ].map(({ name, note }) => (
              <div key={name} className="bg-[#f8fafc] border border-slate-200 rounded-2xl p-6 shadow-sm">
                <div className="flex gap-0.5 mb-3" aria-label="5 étoiles sur 5">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={14} className="text-brand-500 fill-brand-500" aria-hidden="true" />
                  ))}
                </div>
                <p className="text-[#475569] text-sm leading-relaxed mb-4">"{note}"</p>
                <p className="font-heading font-bold text-[#0f172a] text-sm">{name}</p>
              </div>
            ))}
          </div>
        </div>
