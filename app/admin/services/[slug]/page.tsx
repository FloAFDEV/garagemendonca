		...
		>
			...
			>
				<div>
					<span
						className={clsx(
							"font-heading font-normal tracking-widest text-sm",
							t.txt,
						)}
					>
						{title}
			...
			{open && (
				<div
					className={clsx(
						"px-5 sm:px-6 pb-6 pt-2 border-t",
						t.border,
					)}
				>
					{children}
...
export default function EditServicePage({
	...
	const addStep = () =>
		setSteps((p) => [
			...p,
			{ order: p.length + 1, title: "", description: "" },
		]);
	const removeStep = (i: number) =>
		setSteps((p) =>
			p.filter((_, j) => j !== i).map((s, j) => ({ ...s, order: j + 1 })),
		);
	const updateStep = (
		i: number,
	...
	const removeTestimonial = (i: number) =>
		setTestimonials((p) => p.filter((_, j) => j !== i));
	const updateTestimonial = (
		i: number,
		field: keyof TestimonialDraft,
		value: string | number,
	) =>
		setTestimonials((p) =>
			p.map((s, j) => (j === i ? { ...s, [field]: value } : s)),
		...
		<AdminLayout>
			<div className="max-w-2xl mx-auto space-y-6">
				...
				<form onSubmit={handleSubmit} noValidate className="space-y-4">
					...
					>
						<div className="flex items-center justify-between">
							<div>
								<h3
									className={clsx(
										"font-heading font-normal mb-1 tracking-widest text-sm",
										t.txt,
									)}
								>
									Visibilité
								...
								<p className={clsx("text-xs", t.txtSubtle)}>
									Active ou désactive ce service sur la page
									publique.
								</p>
							</div>
							<button
						...
						<div className="space-y-5">
							<div>
								...
								<input
									...
									className={inputClass}
									placeholder="Entret123ien & Révision"
								/>
							</div>
							<div>
								<label className={labelClass}>
									Description courte — home{" "}
									<span className="text-brand-500">*</span>
								</label>
								<textarea
									required
									...
									placeholder="Résumé affiché sur la home et dans les listes…"
								/>
							</div>
							<div>
								<label className={labelClass}>
									Description complète — page services{" "}
									<span className="text-brand-500">*</span>
								</label>
								<textarea
									required
									...
									className={inputClass + " resize-none"}
									placeholder="Description affichée sur la page /services…"
								/>
							</div>
						</div>
					</AdminSection>
					...
					<AdminSection
						title="Prestations"
						subtitle={`${features.length} ligne${features.length > 1 ? "s" : ""}`}
					>
						<div className="space-y-2 mb-3">
							{features.map((feat, idx) => (
								<div
									...
									onDragOver={(e) =>
										handleFeatDragOver(e, idx)
									}
									onDragEnd={() => setFeatDragIdx(null)}
									className={clsx(
										"flex items-center gap-2 group",
										featDragIdx === idx && "opacity-50",
									)}
								>
									<div
										className={clsx(
											"cursor-grab p-1.5 rounded opacity-40 group-hover:opacity-70 transition-opacity",
											t.txtMuted,
										)}
									>
									...
									<button
										...
										className="p-1.5 rounded-lg hover:text-red-400 transition-colors opacity-40 group-hover:opacity-70"
									>
										<Trash2 size={14} />
									</button>
								</div>
									...
									<div className="grid grid-cols-2 gap-3">
										...
										<div>
											...
											<input
												...
												className={inputClass}
												placeholder="à partir de 79 € TTC"
											/>
										</div>
									</div>
									<div>
								...
								>
									...
									<div>
										...
										<textarea
											...
											placeholder="Très bon accueil, travail rapide…"
										/>
									</div>
								</div>
							))}
