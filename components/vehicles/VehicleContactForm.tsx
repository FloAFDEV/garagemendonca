...
import {
  ...
  AlertCircle, User, Mail, PhoneCall,
} from "lucide-react";
import { z } from "zod";
...
export default function VehicleContactForm({
  ...
  const defaultMessage = isAvailable
    ? `Bonjour,\n\nJe suis intéressé(e) par le ${vehicleName} et souhaiterais obtenir plus d'informations.\n\nPuis-je organiser un essai ?\n\nMerci d'avance.`
    : `Bonjour,\n\nJe suis intéressé(e) par le ${vehicleName} (affiché comme vendu).\nSi un véhicule similaire est disponible, merci de me contacter.\n\nMerci.`;
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center text-center py-10 px-6">
        ...
        <h3 className="text-lg font-semibold text-[#0f172a] mb-2">Message envoyé !</h3>
        <p className="text-slate-500 text-sm max-w-xs leading-relaxed">
          Nous avons bien reçu votre demande concernant le{" "}
          <strong className="text-[#0f172a]">{vehicleName}</strong>.
          Nous vous répondrons dans les plus brefs délais — généralement sous 24 h.
        </p>
        <a
          href="tel:0532002038"
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      <div className="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-200 rounded-xl">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm font-semibold text-[#0f172a] truncate">{vehicleLabel}</p>
        </div>
      </div>
      ...
      {/* Message */}
      <div>
        <label htmlFor="vcf-message" className="block text-sm font-medium text-slate-700 mb-1.5">
      ...
      <p className="text-center text-xs text-slate-400">
        ...
        <a href="tel:0532002038" className="text-brand-600 font-medium hover:underline">
          05 32 00 20 38
        </a>
      </p>
    </form>
...
function Field({
  id, name, label, type = "text", required, optional,
  autoComplete, placeholder, icon, error,
}: {
  id: string;
  name: string;
  label: string;
